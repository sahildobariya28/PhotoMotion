...
public class triangleBitmap {
    ...
    private Point f150p1;
    private Point f151p2;
    private Point f152p3;
    ...
    public static ArrayList<triangleBitmap> cutInitialBitmap(Bitmap bitmap) {
        ...
        float f = -1.0f * width2;
        Point ponto = new Point(f, f, true);
        Point ponto2 = new Point(((float) bitmap.getWidth()) + width2, f, true);
        Point ponto3 = new Point(((float) bitmap.getWidth()) + width2, ((float) bitmap.getHeight()) + width2, true);
        Point ponto4 = new Point(f, ((float) bitmap.getHeight()) + width2, true);
        triangleBitmap trianguloBitmap = new triangleBitmap(bitmap, ponto, ponto2, ponto3);
        triangleBitmap trianguloBitmap2 = new triangleBitmap(bitmap, ponto3, ponto4, ponto);
    ...
    public triangleBitmap(Bitmap bitmap, Point ponto, Point ponto2, Point ponto3) {
        ...
        this.f151p2 = ponto2;
        this.f152p3 = ponto3;
        Point[] pontoArr = {ponto, ponto2, ponto3};
        for (int i = 0; i < 3; i++) {
    ...
    public boolean contemVertice(Point ponto, Point ponto2) {
    ...
    public boolean pontoNoCircunscrito(Point ponto) {
        return ponto.naCircunferencia(circuncentro(), this.f150p1.distanciaPara(circuncentro()));
    }

    public Vertex getArestaComum(triangleBitmap trianguloBitmap) {
        if (contemVertice(trianguloBitmap.getP1(), trianguloBitmap.getP2())) {
            return new Vertex(trianguloBitmap.getP1(), trianguloBitmap.getP2());
        }
        if (contemVertice(trianguloBitmap.getP2(), trianguloBitmap.getP3())) {
            return new Vertex(trianguloBitmap.getP2(), trianguloBitmap.getP3());
        }
        if (contemVertice(trianguloBitmap.getP3(), trianguloBitmap.getP1())) {
            return new Vertex(trianguloBitmap.getP3(), trianguloBitmap.getP1());
        }
        return null;
    }

    public boolean eVizinho(triangleBitmap trianguloBitmap) {
    ...
    public Vertex.PacoteDistanciaVertice getVerticeMaisProximo(Point ponto) {
        Vertex.PacoteDistanciaVertice[] pacoteDistanciaVerticeArr = {new Vertex.PacoteDistanciaVertice(new Vertex(this.f150p1, this.f151p2), computeClosestPoint(new Vertex(this.f150p1, this.f151p2), ponto).sub(ponto).mag()), new Vertex.PacoteDistanciaVertice(new Vertex(this.f151p2, this.f152p3), computeClosestPoint(new Vertex(this.f151p2, this.f152p3), ponto).sub(ponto).mag()), new Vertex.PacoteDistanciaVertice(new Vertex(this.f152p3, this.f150p1), computeClosestPoint(new Vertex(this.f152p3, this.f150p1), ponto).sub(ponto).mag())};
        Arrays.sort(pacoteDistanciaVerticeArr);
        return pacoteDistanciaVerticeArr[0];
    }

    private Point computeClosestPoint(Vertex vertice, Point ponto) {
        Point sub = vertice.f154p2.sub(vertice.f153p1);
    ...
    public Point getPontoForaVertice(Vertex vertice) {
    ...
    public double getAnguloPontoOpostoAresta(Vertex vertice) {
        float f;
        Point pontoForaVertice = getPontoForaVertice(vertice);
        float f2 = 0.0f;
        if (pontoForaVertice != null) {
            f2 = Math.abs(new Vertex(pontoForaVertice, vertice.f153p1).coeficienteAngular());
            f = Math.abs(new Vertex(pontoForaVertice, vertice.f154p2).coeficienteAngular());
        } else {
            f = 0.0f;
        }
    ...
    public Point mediana() {
        return new Point(((this.f150p1.getXInit() + this.f151p2.getXInit()) + this.f152p3.getXInit()) / 3.0f, ((this.f150p1.getYInit() + this.f151p2.getYInit()) + this.f152p3.getYInit()) / 3.0f, true);
    }

    public Point circuncentro() {
        Point mediana = this.f150p1.mediana(this.f151p2);
        Point mediana2 = this.f151p2.mediana(this.f152p3);
    ...
    private void desenharVertice(Canvas canvas, Point ponto, Point ponto2, boolean z) {
    ...
    private float sign(Point ponto, Point ponto2, Point ponto3) {
    ...
    public boolean contemPontoDentro(Point ponto) {
        if(ponto != null){
