...
import photo.photomotion.motion.photomotionlivewallpaper.activity.SavingActivity;
...
public class Project implements Parcelable {
    ...
    private List<Point> listaPontos = new CopyOnWriteArrayList();
    private List<Point> listaPontosApresentacao = new CopyOnWriteArrayList();
    ...
    public Project(String str, Bitmap bitmap, Uri uri2) {
        this.titulo = str;
        setImagem(bitmap);
        this.uri = uri2;
        setTempoSave(6000);
    ...
    public Project(long j, String str, Bitmap bitmap, Uri uri2, int i, int i2) {
        this.id = j;
        this.titulo = str;
        if (bitmap != null) {
            setImagem(bitmap);
    ...
    public synchronized void deletePontos(DatabaseHandler databaseHandler, List<Point> list) {
        if (list != null) {
            if (list.size() > 0) {
                this.listaPontos.removeAll(list);
                Long[] lArr = new Long[list.size()];
                for (int i = 0; i < list.size(); i++) {
                    lArr[i] = Long.valueOf(((Point) list.get(i)).getId());
                }
                SQLiteDatabase writableDatabase = databaseHandler.getWritableDatabase();
    ...
    public synchronized void deletePonto(DatabaseHandler databaseHandler, Point ponto) {
    ...
    public synchronized int updatePonto(DatabaseHandler databaseHandler, Point ponto) {
        int update;
        synchronized (this) {
            Point copia = ponto.getCopia(this.proporcaoApresentacao);
            ContentValues contentValues = new ContentValues();
            contentValues.put(Point.COLUMN_XINIT, Float.valueOf(copia.getXInit()));
            contentValues.put(Point.COLUMN_YINIT, Float.valueOf(copia.getYInit()));
            contentValues.put(Point.COLUMN_XFIM, Float.valueOf(copia.getXFim()));
            contentValues.put(Point.COLUMN_YFIM, Float.valueOf(copia.getYFim()));
            contentValues.put(Point.COLUMN_ESTATICO, Integer.valueOf(copia.isEstatico() ? 1 : 0));
            contentValues.put(Point.COLUMN_ID_PROJETO, Long.valueOf(getId()));
            SQLiteDatabase writableDatabase = databaseHandler.getWritableDatabase();
            update = writableDatabase.update(Point.TABLE, contentValues, "id = ?", new String[]{String.valueOf(copia.getId())});
    ...
    public synchronized void addPonto(DatabaseHandler databaseHandler, Point ponto) {
        Point copia = ponto.getCopia(this.proporcaoApresentacao);
        SQLiteDatabase writableDatabase = databaseHandler.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(Point.COLUMN_XINIT, Float.valueOf(copia.getXInit()));
        ...
        contentValues.put(Point.COLUMN_ESTATICO, Integer.valueOf(copia.isEstatico() ? 1 : 0));
        contentValues.put(Point.COLUMN_ID_PROJETO, Long.valueOf(getId()));
        ponto.setId(writableDatabase.insert(Point.TABLE, null, contentValues));
    ...
    public synchronized void carregarPontos(DatabaseHandler databaseHandler) {
        ...
        SQLiteDatabase readableDatabase = databaseHandler.getReadableDatabase();
        SQLiteDatabase sQLiteDatabase = readableDatabase;
        Cursor query = sQLiteDatabase.query(Point.TABLE, new String[]{"id", Point.COLUMN_ESTATICO, Point.COLUMN_XINIT, Point.COLUMN_YINIT, Point.COLUMN_XFIM, Point.COLUMN_YFIM}, "id_projeto=?", new String[]{String.valueOf(this.id)}, null, null, null, null);
        if (query.moveToFirst()) {
            do {
                if (query.getInt(1) == 1) {
                    ponto = new Point(query.getFloat(2), query.getFloat(3), true);
                } else {
                    ponto = new Point(query.getFloat(2), query.getFloat(3), query.getFloat(4), query.getFloat(5));
                }
    ...
    public Project(Parcel parcel) {
        ...
        parcel.readList(this.listaPontos, Point.class.getClassLoader());
    }
